...
- [ ] **BUG (regression from the 624d88c re-nesting, found 2026-09-03): `CONFIG_PATH` resolves to a non-existent path** `[SONNET]` - `spotify_tools/config.py`: `BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))` was written when `config.py` lived at `spotify_tools/config.py` (repo root), so two `dirname()` calls landed on the repo root. After the re-nesting `__file__` is now `src/spotify_tools/config.py`, one level deeper, so the same two `dirname()` calls land on `src/` instead - `CONFIG_PATH` becomes `<repo>/src/config/config.json`, which does not exist; the real file is at `<repo>/config/config.json`. Verified directly: `python -c "from spotify_tools.config import CONFIG_PATH; import os; print(CONFIG_PATH, os.path.exists(CONFIG_PATH))"` (with `src/` on `sys.path`) prints the wrong path and `False`. `load_config()` fails silently (file-not-found -> `{}`, no exception), so anything depending on it looks like "no config" rather than "wrong path" until traced. Neither test suite caught it: no test in this repo asserts `CONFIG_PATH` points to a real file, and AudioManager's `test_acquire.py` never exercises `_spotify_client()` (its `spotify_tools.*` imports are deferred inside function bodies) - "both suites pass" in the note above did not cover this path at all. **Confirmed downstream impact:** AudioManager's Acquire tab is currently broken end-to-end for every Spotify-backed action (Fetch Playlist, Sync Liked, anything through `_spotify_client()`), raising `RuntimeError("SpotifyTools config not found at ...")`; tracked on that side too (AudioManager `docs/Development/IDEAS.md` TIER 2). Fix: add one more `dirname()` call (or better, stop relying on relative-path arithmetic entirely and use the same `SPOTIFY_TOOLS_ROOT`-env-var-with-fallback approach already proposed in Task 2 above, since that fixes this exact class of fragility at the source instead of patching one more layer of `dirname()`). Add a regression test asserting `os.path.exists(CONFIG_PATH)` (or an equivalent that doesn't require real credentials) so a future re-nesting can't reintroduce this silently. Done when `CONFIG_PATH` resolves correctly, a test guards it, and AudioManager's Acquire tab is manually re-verified against a real playlist id.
